...
def hardware_thread(end_event, hw_queue) -> None:
  ...
  while not end_event.is_set():
    ...
    startup_conditions["accepted_terms"] = True # params.get("HasAcceptedTerms") == terms_version
    startup_conditions["accepted_terms_sp"] = True # params.get("HasAcceptedTermsSP") == terms_version_sp

    # with 2% left, we killall, otherwise the phone will take a long time to boot
    startup_conditions["free_space"] = msg.deviceState.freeSpacePercent > 2
    startup_conditions["completed_training"] = True # params.get("CompletedTrainingVersion") == training_version
